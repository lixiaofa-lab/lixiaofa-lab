    ...
    <div class="main-panel">
      ...
      <div class="earth-panel">
        ...
        <div class="earth-tool">
          <div class="earth-tool-body">
            <div class="item-btn shine-box" @click="warnInfo">异常预测</div>
            <div class="item-btn shine-box" @click="pointPointBlock">数据查询</div>
            <div class="item-btn shine-box" @click="">数据统计</div>
          </div>
        </div>
      </div>
      ...
      <div class="info-aside">
        <div class="info-aside-top">
              ...
              <CommunicationImportanceStatistics/>

        </div>
        <div class="info-aside-center">

              <ElectricityImportanceStatistics/>
              <!--                    <Stack/>-->
              <PetrifactionImportanceStatistics/>
        </div>
        <div class="info-aside-bottom shine-box">
            <ImportanceSort/>
        </div>
      </div>
    </div>
    ...
    <el-dialog title="重要性分析" :visible.sync="importance">
      <el-form :model="form1">
        <el-form-item label="设施名称：" :label-width="form1LabelWidth">
          <el-input @input="imp()" v-model="form1.name" auto-complete="off" placeholder="请输入设施名称："></el-input>
        </el-form-item>
...
<!--    </el-dialog>-->
    ...
    <!--  异常预警弹框-->
    <el-dialog title="自然灾害预警信息" :visible.sync="warning">
      <el-form :model="form3">
...
<script>
  ...
  import ImportanceTarget from 'src/EchartsClassify/importanceTarget'
  import ImportanceStatistics from 'src/EchartsClassify/importanceStatistics'
  import History from 'src/EchartsClassify/history'

  import TrafficImportanceStatistics from 'src/EchartsClassify/trafficImportanceStatistics'
  import CommunicationImportanceStatistics from 'src/EchartsClassify/communicationImportanceStatistics'
  import ElectricityImportanceStatistics from 'src/EchartsClassify/electricityImportanceStatistics'
  import PetrifactionImportanceStatistics from 'src/EchartsClassify/petrifactionImportanceStatistics'
  import ImportanceSort from 'src/EchartsClassify/importanceSort'
  ...
  export default {
    ...
    components: {
      ...
      EntityInfo,
      // Echarts图形组件
      BaseInfo,Sort,ImportanceTarget,ImportanceStatistics,History,
      TrafficImportanceStatistics, CommunicationImportanceStatistics,
      ElectricityImportanceStatistics,PetrifactionImportanceStatistics,
      ImportanceSort,
    },
    ...
    methods: {
      ...
      getPointsData () {
        this.axios.get('/facility/gs').then((res) => {
          ...
          this.pointsData = Array.from(data, item => {
            let {id, lon: lon, lat: lat, name, type,grade,score,import: imp,frag:fra,} = item
            return {id, lon, lat, name, type,grade,score, imp,fra}
          })
        })
      },
      //设施点上球
      loadPointsDataToEarth () {
        ...
        if (points.length > 0 && earthIsLoaded == 1) {
          ...
          for (let item of points) {
            ...
            let {id, lon, lat, name,type,grade,score,imp,fra} = item
            window.GLOBAL_WINDOW['add_impscore']({id, lon, lat, name,score,imp,fra})
            window.GLOBAL_WINDOW['addPoints_zdpx']({id, lon, lat, name,imp,fra})
      ...
      closeImportanceHandle () {
        ...
        if(this.form1.name == '' && this.form1.region == '3'){
          let data = {
            code:"FDC002",
            name:"临沧发电厂",
            type:"电力设施",
        ...
        if(this.form1.name == '' && this.form1.region == '1'){
          let data = {
            code:"JC005",
            name:"百色机场",
            type:"交通设施",
        ...
        this.axios.get(`/facility/importValue/${type}/${name}/`).then((res) => {
          let {code,data, msg} = res
          if (code !== 0) {
            // this.$message({
            //   type: 'warning',
          ...
          if(type == '电力设施'){
            window.GLOBAL_WINDOW['addPoints_zdpx']({lat, lon,importance,type,name})
            window.GLOBAL_WINDOW['ImportanceBlue']({lat, lon,importance,type,name})
            window.GLOBAL_WINDOW['add_electricity']({lat, lon,importance,type,name})
            window.GLOBAL_WINDOW['add_impscore']({lat, lon,imp,type,name})
            window.GLOBAL_WINDOW['fly']({lat, lon,importance,type,name})
          }
          if(type == '交通设施'){
            window.GLOBAL_WINDOW['addPoints_zdpx']({lat, lon,importance,imp,type,name})
            window.GLOBAL_WINDOW['ImportanceBlue']({lat, lon,importance,type,name})
            window.GLOBAL_WINDOW['add_electricity']({lat, lon,importance,type,name})
            window.GLOBAL_WINDOW['add_impscore']({lat, lon,imp,type,name})
            window.GLOBAL_WINDOW['fly']({lat, lon,importance,type,name})
            ...
            for (let item of data) {
              ...
              if (data.length > 1) {
                if (item.isabnormal === '是') {
                  ...
                  // window.GLOBAL_WINDOW.removeAllEntities()
                  window.GLOBAL_WINDOW['disasterAbnormal'](data, lat, lon)
                  window.GLOBAL_WINDOW['add_electricity']({lat, lon})
                  window.GLOBAL_WINDOW['addPoints_zdpx']({lat,lon,name:item.name})
                } else {
                  // this.$message({
                  //   type: 'warning',
                  //   message: '无灾害异常'
